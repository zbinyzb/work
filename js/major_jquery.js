$(function (){//在DOM加载完成后运行预定函数
    var url = "http://10.0.78.25:8624/goushushenpixitong/test/major"//请求URL
    var laboratory;
    ...
    $("#getOne").click(function () {    //点击id = getOne标签执行
     ...
     //添加专业
     $.post(url,
        {id: id, name: name, instituteId: instituteId, remarks: remarks},//相关参数
        function(data){
            alert("添加成功");
            console.log(data.id);
    ...
    $.ajax({
        type: 'PUT',
        url: url,
        data: {id:id, name: name, instituteId: instituteId, remarks: remarks},
